...
- **MCP条文锚定**：72个法规锚点，每次运行通过用户配置的MCP后端（优先华宇元典）实时核验法规现行有效性，避免引用已失效或已修订条款。
- **双后端能力槽**：法规核验MCP后端可插拔配置，首选华宇元典（需API Key），备选其他已连接的法律检索MCP，见 `assets/mcp-backends.yaml`。
...
├── references/               # 深度文档
│   ├── law-library.md        # 法规依据库（24部法规，经MCP后端核验）
...
├── assets/                   # 配置与输出模板
│   ├── mcp-backends.yaml           # MCP法规核验后端配置（优先yuandian，备选其他法律检索MCP）
│   ├── check-results-schema.md     # 检查结果JSON数据结构文档
...
3. **法规实时核验**：数据合规领域法规更新频繁，skill每次运行通过MCP后端实时核验引用法规的现行有效性。
